...
import { ArrowRight, Github, Twitter, ArrowUpRight, MoveDown, ArrowDownLeft } from 'lucide-react';
import { PROFILE, PROJECTS, SKILLS } from '../data/constants';
...
const MinimalView: React.FC<MinimalViewProps> = ({ onSwitch, onSelectProject }) => {
  // Duplicate skills multiple times for smooth marquee
  const marqueeSkills = [
  ...
  const services = [
    { title: "Frontend Arch.", description: "Scalable React & Next.js applications" },
    { title: "Interaction", description: "WebGL & Framer Motion animations" },
    { title: "Backend Systems", description: "Robust Node.js & Go APIs" },
    { title: "Design Systems", description: "Consistent, accessible UI libraries" }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] font-sans text-stone-900 selection:bg-stone-900 selection:text-white overflow-x-hidden relative">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 p-6 md:p-8 flex justify-between items-center pointer-events-none">
        ...
        >
            <span className="font-display font-bold text-xl tracking-tight text-stone-900">N.</span>
        </motion.div>
      </nav>
      ...
      >
        <div className="flex flex-col gap-6">
            <div className="flex items-center gap-4 mb-4">
                ...
                >
                    <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${PROFILE.name}&backgroundColor=b6e3f4`} alt="Avatar" className="w-full h-full bg-stone-50" />
                </motion.div>
                <motion.div
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest text-stone-400">Online</span>
                    </div>
                    <span className="text-sm font-medium text-stone-900">San Francisco, CA</span>
                </motion.div>
            </div>
         ...
         <div className="flex whitespace-nowrap w-max animate-marquee">
            {marqueeSkills.map((skill, i) => (
                <div key={i} className="flex items-center mx-8 md:mx-16 opacity-30">
                    ...
                    <span className="ml-16 md:ml-32 text-4xl md:text-6xl text-stone-300">✦</span>
                </div>
            ))}
         </div>
      </div>
                 ...
                 {services.map((service, i) => (
                     ...
                     >
                         <div>
                            ...
                            <p className="text-stone-400 group-hover:text-stone-600 transition-colors">{service.description}</p>
                         </div>
      ...
      <section className="bg-stone-900 text-stone-100 py-32 rounded-[3rem] mx-2 md:mx-4">
        <div className="max-w-[1600px] mx-auto px-6 md:px-12">
            <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-24 border-b border-white/10 pb-12">
                <h2 className="text-[12vw] md:text-[8vw] font-display font-bold leading-none tracking-tighter text-white">Works</h2>
                <div className="flex items-center gap-3 mb-2 md:mb-6">
                   <div className="flex -space-x-3">
                        ...
                        <div className="w-10 h-10 rounded-full border-2 border-stone-900 bg-stone-500 flex items-center justify-center text-xs font-bold">5+</div>
                   </div>
                   <span className="text-sm font-bold uppercase tracking-widest text-stone-400">Selected Projects</span>
                </div>
            </div>
                        ...
                        <div className="md:col-span-6">
                            ...
                            <div className="flex gap-2">
                                {project.tags.slice(0,3).map(tag => (
                                    <span key={tag} className="px-3 py-1 rounded-full border border-white/20 text-xs text-stone-400 uppercase tracking-wider">
                                        {tag}
                                    </span>
                                ))}
                            </div>
      ...
      <footer className="pt-40 pb-12 px-6 md:px-12 max-w-[1600px] mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-20 mb-32">
                ...
                <div className="flex flex-col justify-end items-start md:items-end gap-10">
                    ...
                    >
                        ...
                        <ArrowDownLeft className="group-hover:rotate-180 transition-transform duration-500" size={40} />
                    </a>
                    <div className="flex gap-4">
                        {[
                            ...
                            <a key={i} href={Item.href} className="w-16 h-16 rounded-full bg-stone-100 flex items-center justify-center text-stone-900 hover:bg-stone-900 hover:text-white transition-all duration-300">
                                <Item.icon size={24} />
                            </a>
                        ))}
                    </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center text-xs md:text-sm text-stone-400 font-bold uppercase tracking-widest pt-8 border-t border-stone-200">
                ...
                <div className="flex gap-8 mt-4 md:mt-0">
                    <span className="hover:text-stone-900 cursor-pointer transition-colors">Linkedin</span>
                    <span className="hover:text-stone-900 cursor-pointer transition-colors">Twitter</span>
                    <span className="hover:text-stone-900 cursor-pointer transition-colors">Instagram</span>
                </div>
            </div>
      </footer>
    </div>
